...
const VALIDATED_DESIGN_CATEGORIES = new Set<CanonicalToken["category"]>([
  "color",
  "spacing",
  "radius",
]);

export function validateDesignTokensAgainstCss(
  designTokens: CanonicalToken[],
  cssTokens: CanonicalToken[]
): TokenValidationReport {
  const findings: TokenValidationFinding[] = [];

  const cssTokenMap = new Map(
    cssTokens
      .filter((t) => t.source === "css")
      .map((t) => [normalizeCssVarName(t.name), t] as const)
  );

  const expectedCssVars = new Set<string>();

  for (const token of designTokens) {
    if (token.source !== "design-md") continue;
    if (!VALIDATED_DESIGN_CATEGORIES.has(token.category)) continue;
  ...
  for (const cssToken of cssTokens) {
    if (cssToken.source !== "css") continue;
    if (!VALIDATED_DESIGN_CATEGORIES.has(cssToken.category)) continue;
...
export function validateMissingDesignTokensInCss(
  designTokens: CanonicalToken[],
  cssTokens: CanonicalToken[]
): TokenValidationReport {
...
function buildReport(findings: TokenValidationFinding[]): TokenValidationReport {
...
function designTokenToCssVarCandidates(path: string): string[] {
  const parts = path.split(".");
  if (parts.length < 2) return [toKebab(parts.join("-"))];
